...
class Reconstructor(nn.Module):
    ...
    def forward(self, h):
        # h shape: [Batch_Total, Num_Tokens, d_token] 
        # (여기서 Batch_Total은 B * S 일 수 있음)

        # 1. 수치형과 범주형 토큰 분리
        h_num = h[:, :self.d_numerical] # [Batch_Total, d_numerical, d_token]
        h_cat = h[:, self.d_numerical:] # [Batch_Total, len(categories), d_token]

        # 2. 수치형 복원 (Numerical Reconstruction)
        # self.weight: [d_numerical, d_token] -> [1, d_numerical, d_token]으로 확장하여 곱함
        # 이를 통해 Batch_Total 차원에 대해 동일한 가중치로 브로드캐스팅 연산 수행
        recon_x_num = torch.mul(h_num, self.weight.unsqueeze(0)).sum(-1) # [Batch_Total, d_numerical]

        # 3. 범주형 복원 (Categorical Reconstruction)
        recon_x_cat = []
        for i, recon in enumerate(self.cat_recons):
            # h_cat[:, i] shape: [Batch_Total, d_token]
            # recon(h_cat[:, i]) shape: [Batch_Total, categories[i]]
